...
public class nurbs_ct_surfs implements Serializable {
    ...
    public void draw (GLUnurbsObj nobj, GL JavaGL, int display_mode,
	...
	for (j = 0; j < s_bezier_cnt; j++) {
		...
		if (!nobj.culling_test_3d (geom.offsets [j][i],
		    ...
		    if (do_color)  { color.draw  (JavaGL,surface.color,  j,i); }
		    if (do_normal) { normal.draw (JavaGL,surface.normal, j,i); }
		    if (do_texture){ texture.draw(JavaGL,surface.texture,j,i); }
		    n_map_b.map (j, i);
		}
	    }
	}
