...
public class gl_feedback implements Serializable {

    private static final long serialVersionUID = 5124587295520328189L;
...
//	if ((Mask & FB_INDEX) != 0 ) { write_feedback_token (index); }
	if ((Mask & FB_COLOR) != 0 ) {
	    ...
	    color [0] = ((c & 0x00ff0000) >> 16) / (float)255.0;
	    color [1] = ((c & 0x0000ff00) >> 8) / (float)255.0;
	    color [2] = (c & 0x000000ff) / (float)255.0;
	    color [3] = ((c & 0xff000000) >> 24) / (float)255.0;
