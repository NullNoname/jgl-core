...
public class nurbs_ct_curvs implements Serializable {

    private static final long serialVersionUID = 2558057409442251088L;
    ...
    public void draw (GLUnurbsObj nobj, GL JavaGL,
		      nurbs_curves curve, int factors []) {
	...
	for (i = 0; i < bezier_cnt; i++) {
		...
		if (do_color)  { color.draw  (JavaGL,curve.color,  i); }
		if (do_normal) { normal.draw (JavaGL,curve.normal, i); }
		if (do_texture){ texture.draw(JavaGL,curve.texture,i); }
		n_map_b.map (i);
	    }
	}
    }
