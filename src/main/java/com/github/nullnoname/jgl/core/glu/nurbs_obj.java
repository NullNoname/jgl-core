...
public class nurbs_obj implements Serializable {
	...
	int i;
        ...
        for (i = 1; i < knot_count; i++) {
            if (knot [i] < temp)  { return GLU.GLU_NURBS_ERROR4; }
	    if (temp == knot [i]) { mult++; }
	    else {
                if (mult > order) { return GLU.GLU_NURBS_ERROR5; }
                mult = 1; temp = knot [i];
            }
        }
    ...
    public int test () {
        if (order < 0)		     { return GLU.GLU_INVALID_VALUE; }
	// assume MAX_EVAL_ORDER = 30
        if (order > 30 || order < 2) { return GLU.GLU_NURBS_ERROR1;  }
	if (knot_count < order + 2)  { return GLU.GLU_NURBS_ERROR2;  }
	if (stride < 0) 	     { return GLU.GLU_NURBS_ERROR34; }
	if (knot == null)	     { return GLU.GLU_NURBS_ERROR36; }
	return knot_test ();
    }
