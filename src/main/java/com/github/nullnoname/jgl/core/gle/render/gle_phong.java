...
public class gle_phong extends gl_depth implements Serializable {
	...
	nx = LineN[0][0]; ny = LineN[0][1]; nz = LineN[0][2];
	px = LineP[0][0]; py = LineP[0][1]; pz = LineP[0][2];
    }
    ...
    protected void phong_inc_x () { nx+=dnxdx; ny+=dnydx; nz+=dnzdx;
				    px+=dpxdx; py+=dpydx; pz+=dpzdx; }
    protected void phong_dec_x () { nx-=dnxdx; ny-=dnydx; nz-=dnzdx;
				    px-=dpxdx; py-=dpydx; pz-=dpzdx; }
    protected void phong_inc_y () { nx+=dnxdy; ny+=dnydy; nz+=dnzdy;
				    px+=dpxdy; py+=dpydy; pz+=dpzdy; }

    protected void inc_x () { x_inc_x (); phong_inc_x (); }
    protected void dec_x () { x_dec_x (); phong_dec_x (); }
    protected void inc_y () { y_inc_y (); phong_inc_y (); }
	...
	LeftPointNX += dnxdxy; LeftPointNY += dnydxy; LeftPointNZ += dnzdxy;
	LeftPointPX += dpxdxy; LeftPointPY += dpydxy; LeftPointPZ += dpzdxy;
    }
	...
	LeftPointNX += dnxdyy; LeftPointNY += dnydyy; LeftPointNZ += dnzdyy;
	LeftPointPX += dpxdyy; LeftPointPY += dpydyy; LeftPointPZ += dpzdyy;
    }
	...
	draw_horizontal_line (LeftPoint,   RightPoint,  y,
			      LeftPointNX, LeftPointNY, LeftPointNZ,
